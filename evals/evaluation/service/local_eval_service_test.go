...
func TestPerformInferenceRunSessionStateOnlyNoCaseInput(t *testing.T) {
	ctx := context.Background()
	sessSvc := session.InMemoryService()
	const wantSessionID = generator.EvalSessionIDPrefix + "run-state-only"
	...
	if idea != "ideas/run-only" {
		t.Fatalf("idea_name = %v, want ideas/run-only", idea)
	}
}
