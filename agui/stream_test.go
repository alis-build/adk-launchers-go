...
func TestProcessEvent_StepEvents(t *testing.T) {
	...
	evts2 := parseSSEEvents(rec2.Body.String())
	if len(evts2) < 2 {
		t.Fatalf("got %d events, want at least 2 (TEXT_MESSAGE_END + STEP_FINISHED)", len(evts2))
	}
	// The open text message from the sub-agent partial is closed before the
	// step transition so the next partial opens a fresh TEXT_MESSAGE_START
...
func TestProcessEvent_AuthorSwitchMidStream_ClosesAndReopensText(t *testing.T) {
	...
	ev1.Content = genai.NewContentFromText("Hello", genai.RoleModel)
	ev1.Partial = true
	if _, err := l.processEvent(e, ev1, state); err != nil {
		t.Fatalf("processEvent() error = %v", err)
	}
