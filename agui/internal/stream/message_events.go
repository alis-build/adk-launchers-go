...
// TextMessageStartEvent is an emit-only shim matching the upstream wire shape until
// the AG-UI Go SDK adds optional Name on events.TextMessageStartEvent. Delete this
// type and use events.NewTextMessageStartEvent(..., events.WithName(...)) after
// upstream lands.
//
// Type-assertion note: [CallInterceptor.OnEmit] implementations that inspect
// TEXT_MESSAGE_START will receive *stream.TextMessageStartEvent, not the upstream
